...
void slab_grow(struct slab_allocator *slabs, void *buf, size_t buflen)
{
    if (buflen <= sizeof(struct slab_head)) {
        printf("%s: slabs=%p, buf=%p, buflen=%zu sizeof(slab_head) = %zu\n",
...
void *slab_alloc(struct slab_allocator *slabs)
{
    ...
    if (sh == NULL) {
        ...
        if (!slabs->refill_func) {
            ...
            if (err_is_fail(err)) {
                printf("slab refill_func failed %d \n", err);
                return NULL;
            }
            for (sh = slabs->slabs; sh != NULL && sh->free == 0; sh = sh->next);
            if (sh == NULL) {
...
{
    return slab_refill_pages(slabs, PAGE_SIZE);
}
