...
static errval_t loopback_dequeue(struct cleanq* q, regionid_t* rid,
...
#if 0
        err = cleanq_dequeue(q, &(buf.rid), &(buf.addr), 
                           &(buf.len), &(buf.bid), &(buf.flags));
        if (err_is_fail(err))  {
            return err;
        }   

        err = cleanq_enqueue(q, buf.rid, buf.addr, buf.len,
                           buf.flags, &buf.bid);
        if (err_is_fail(err))  {
            return err;
        }   
...
static errval_t loopback_register(struct cleanq *q, struct capref cap,
...
static errval_t loopback_control(struct cleanq *q,
                                 uint64_t request,
                                 uint64_t value,
                                 uint64_t *result)
{
